...
def run_sensitivity_analysis(parameters, model_calculation, num_samples):
    ...
    Y = np.array([try_eval(model_calculation(**dict(zip(parameters.keys(), vs)))) for vs in param_values])
...
def staff_aggregates_to_params(nums, percent):
    return { k: log_normal_params_from_90_percent_ci(**create_bounds(v, percent)._asdict()) for k, v in nums.items() }
